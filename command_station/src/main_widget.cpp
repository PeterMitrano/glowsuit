...
QMediaPlayer::State MainWidget::state() const { return player_state; }
...
void MainWidget::any_event() { blink_midi_indicator(); }
...
void MainWidget::sendTime(qint64 song_time_ms) {
  ...
  // we want to send a proper xbee packet here, just like we'd send to the real
  // suits
