...
Serial::SerialImpl::reconfigurePort()
{
    ...
    if (tcgetattr(fd_, &options) == -1)
    {
        ...
        {
            case EBADF:
                THROW (IOException, "The fildes argument is not a valid file descriptor.");
            case ENOTTY:
                THROW(IOException,
                      "The file associated with fildes is not a terminal. The tcsetattr() function may fail if:");
            default:
                THROW(IOException,
                      "Unknown tcgetattr error");
        }
    }
